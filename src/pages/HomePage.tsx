    ...
    return (
      <AppLayout>
        <div className="animate-pulse space-y-8">
          <div className="h-48 bg-slate-200/50 rounded-3xl" />
      ...
      <div className="space-y-8">
        <div className="relative overflow-hidden rounded-3xl bg-slate-900 p-8 md:p-12 text-white shadow-2xl">
          <div className="relative z-10 max-w-2xl">
            <Badge variant="outline" className="mb-4 border-emerald-500/50 text-emerald-400 font-mono uppercase tracking-widest text-[10px]">Live Intelligence</Badge>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="border-none shadow-sm bg-white hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-slate-900">{stats.totalVolume}</div>
              <p className="text-[10px] text-emerald-600 font-bold flex items-center mt-2 uppercase tracking-tighter">
                <ArrowUpRight className="size-3 mr-1" /> Real-time aggregate
              </p>
            </CardContent>
          </Card>
          <Card className="border-none shadow-sm bg-white hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-slate-900">{stats.activeTraders}</div>
          ...
          <Card className="border-none shadow-sm bg-white hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-slate-900 truncate">{stats.topSector}</div>
              <p className="text-[10px] text-slate-500 font-bold mt-2 uppercase tracking-tighter">
                ...
                {stats.recentTrades.map((trade) => (
                  <div key={trade.id} className="flex items-center gap-4 px-4 sm:px-0">
                    <div className="relative flex-shrink-0">
                      <img src={trade.politician.avatarUrl} className="size-11 rounded-full bg-slate-50 border border-slate-100" />
                      <div className={`absolute -bottom-0.5 -right-0.5 size-3.5 rounded-full border-2 border-white ${trade.type === 'Buy' ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-bold text-slate-900 truncate">{trade.politician.name}</p>
                        <span className={`text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ${trade.type === 'Buy' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                          {trade.type}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant="outline" className="px-1.5 py-0 text-[10px] font-mono rounded-sm border-slate-200">{trade.issuerId}</Badge>
